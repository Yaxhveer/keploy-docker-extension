...
```shell
  docker buildx build -t my/keploy:latest . --load
```

To install the extension, use `make install-extension` **or**:

```shell
  docker extension install my/keploy:latest
```
...
```shell
  docker extension dev ui-source my/keploy:latest http://localhost:3000
```

In order to open the Chrome Dev Tools for your extension when you click on the extension tab, run:

```shell
  docker extension dev debug my/keploy:latest
```

Each subsequent click on the extension tab will also open Chrome Dev Tools. To stop this behaviour, run:

```shell
  docker extension dev reset my/keploy:latest
```
...
```shell
docker extension update my/keploy:latest
```
...
```shell
docker extension rm my/keploy:latest
```
